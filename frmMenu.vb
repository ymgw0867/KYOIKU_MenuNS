Public Class frmMenu
    ...
    Const JOB_SIWAKE As String = "勘定奉行個別原価管理"
...
ErrPrc:
    ...
    Private Sub CmdDetail(ByVal mode As Integer, ByVal JobName As String, ByVal StartEXE As String)
        ...
        If mode = 0 Then
            ...
            'str = pblWinReaderPath & "\\" & WRHAND_PRG & " """ & JobName & """ /h2"
            str = pblWinReaderPath & "\" & WRHAND_PRG & " """ & JobName & """ /h2"
            'retVal = fncOpenExe(pblWinReaderPath & "\\" & WRHAND_PRG & " """ & JobName & """ /h2", "伝票スキャン")
            'MsgBox(str)
            retVal = fncOpenExe(str, "伝票スキャン")
            Me.Cursor = Cursors.Default   ' マウス ポインタを元に戻します。
        End If
    ...
    Private Sub frmMenu_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'アプリケーション作業ディレクトリ(インストール先パス）
        pblInstPath = My.MySettings.Default.instDir     '設定ファイルにて保持する 2011/3/22

        'WinReaderHands S インストール先パス　
        'pblWinReaderPath = WrHandInstPath()
        pblWinReaderPath = My.MySettings.Default.instWinReader  '設定ファイルにて保持する 2011/3/23
        ...
        If Dir(pblInstPath & DIR_HENKAN & "勘定奉行*.*") <> "" Then
            ...
            FileCopy(pblInstPath & DIR_HENKAN & SFileName & ".tif", pblWinReaderPath & "\\TEMPLATE\\" & SFileName & ".tif")
            Kill(pblInstPath & DIR_HENKAN & SFileName & ".tif")
...
ErrPrc:
    ...
    Private Sub cmdKotei_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdKotei.Click
        CmdDetail(0, "入出金伝票固定項目設定", PRGKOTEI)
    End Sub
End Class
